...
class CoordinatorThread(QThread):
    ...
    def _bot_decide(self, arena: dict) -> dict | None:
        self._ensure_bot()
        if self._bot_proc is None or self._bot_proc.stdin is None or self._bot_proc.stdout is None:
            return None
        try:
            self._bot_proc.stdin.write(json.dumps(arena, ensure_ascii=False) + "\n")
